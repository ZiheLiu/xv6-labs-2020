...
// and enable paging.
void kvminithart() {
  proc_kvminithart(kernel_pagetable);
}
...
{
  proc_kvmmap(kernel_pagetable, va, pa, sz, perm);
}
...
void proc_freewalk(pagetable_t pagetable, int level) {
  for(int i = 0; i < 512; i++){
    if (level == 0) {
      pagetable[i] = 0;
      continue;
...
// Copy user page table to kernel page table with pages in address [start_va, end_va].
int proc_kernel_uvmcopy(pagetable_t dst, pagetable_t src, uint64 start_va, uint64 end_va) {
  uint64 i;
  pte_t *pte;
  uint64 pa;
  uint flags;
  ...
  for (i = start_va; i < end_va; i += PGSIZE) {
    ...
    if (proc_mappages(dst, i, PGSIZE, pa, flags) != 0) {
      printf("proc_kernel_uvmcopy err\n");
      goto err;
    }
  }
  return 0;
  ...
  err:
  uvmunmap(dst, start_va, (i - start_va) / PGSIZE, 0);
  return -1;
}
...
void vmprint_walk(pagetable_t pagetable, int deep) {
  for(int i = 0; i < 512; i++) {
    pte_t pte = pagetable[i];
    if(pte & PTE_V){
      uint64 pa = PTE2PA(pte);
      ...
      // print pte.
      printf("%d: pte %p pa %p\n", i, pte, pa);
      // print child page table.
      if (deep < 3) {
        vmprint_walk((pagetable_t)pa, deep + 1);
      }
    }
  }
}
